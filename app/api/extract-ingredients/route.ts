  ...
  const extractIngredients = async () => {
    try {
      ...
      ingredients = ingredients.filter((ing) =>
        shouldIncludeIngredient(ing.name)
      );

      await sendLog(
        `Found ${ingredients.length} ingredients after filtering excluded ones`
      );
      ...
      for (const ing of ingredients) {
        try {
          ...
          await sendLog(
            `Error processing ingredient ${ing.name}: ${
      ...
      await writer.write(
        encoder.encode(JSON.stringify({ ingredients }) + "\n")
      );
    } catch (error) {
