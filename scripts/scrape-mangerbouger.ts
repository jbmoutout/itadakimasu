...
async function translateIngredients(
  ...
  const namesToTranslate = frenchNames.filter(
    (name) => !translations.has(name.toLowerCase())
  );
...
async function tryUrl(
  ...
  await page.goto(
    `https://www.mangerbouger.fr/manger-mieux/bien-manger-sans-se-ruiner/calendrier-de-saison/les-fruits-et-legumes-${urlFormat}-${monthUrl}`,
    {
      timeout: 60000,
    }
  );
...
async function saveSeasonalData(
  ingredientMonths: Map<string, number[]>,
...
async function main() {
  try {
    ...
    const translations = await translateIngredients(
      Array.from(ingredientMonths.keys())
    );
